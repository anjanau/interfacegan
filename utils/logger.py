...
def setup_logger(work_dir=None, logfile_name='log.txt', logger_name='logger'):
  ...
  if logger.hasHandlers():  # Already existed
    raise SystemExit('Logger name `{logger_name}` has already been set up!\n'
                     'Please use another name, or otherwise the messages '
                     'may be mixed between these two loggers.')
  ...
  if os.path.exists(work_dir):
    raise SystemExit('Work directory `{work_dir}` has already existed!\n'
