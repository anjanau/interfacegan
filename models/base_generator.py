...
class BaseGenerator(object):
  ...
  def check_attr(self, attr_name):
    ...
    if not hasattr(self, attr_name):
      raise AttributeError(
          '`{attr_name}` is missing for model `{self.model_name}`!')
  ...
  def get_value(self, tensor):
    ...
    raise ValueError('Unsupported input type `{type(tensor)}`!')
  ...
  def postprocess(self, images):
    ...
    if len(images_shape) != 4 or images_shape[1] not in [1, 3]:
      raise ValueError('Input should be with shape [batch_size, channel, '
                       'height, width], where channel equals to 1 or 3. '
                       'But {images_shape} is received!')
